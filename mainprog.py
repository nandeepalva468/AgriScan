...
@app.route('/predict1', methods=['POST'])  # 🟢 Leaf Disease
def predcrop1():
    if request.method == 'POST':
        import subprocess
        subprocess.call("python -m streamlit run main1.py", shell=True)  # ✅ main1.py ← LEAF
@app.route('/predictinfo1')
def predictin1():
   return render_template('info1.html')
@app.route('/predict', methods=['POST'])  # 🟢 Weed Detection
def predcrop():
    if request.method == 'POST':
        import subprocess
        subprocess.call("python -m streamlit run main3.py", shell=True)  # ✅ main3.py ← WEED
